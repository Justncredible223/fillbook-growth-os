 ...
 * inspection during a release-readiness audit: this screen's "Nothing here
 * yet" empty state used to sit entirely outside PullToRefreshBox, so
 ...
 */
class XFeedPostHistoryScreenEmptyStateStructureTest {
    private fun screenSource(): String {
        val candidates = listOf(
            "src/main/java/com/fillbook/growthos/ui/screens/XFeedPostHistoryScreen.kt",
            "app/src/main/java/com/fillbook/growthos/ui/screens/XFeedPostHistoryScreen.kt",
        )
        val file = candidates.map { java.io.File(it) }.firstOrNull { it.exists() }
            ?: error(
                "Could not locate XFeedPostHistoryScreen.kt from working directory " +
    ...
    fun `the empty-state branch is inside PullToRefreshBox and wraps PolishedEmptyState in a LazyColumn`() {
        ...
        val emptyBranchIndex = source.indexOf("errorMessage == null && entries.isEmpty()")
        check(emptyBranchIndex >= 0) { "Could not find the empty-state condition in XFeedPostHistoryScreen.kt -- has this branch been restructured?" }
        ...
        check(pullToRefreshIndex in 0 until polishedEmptyStateIndex) {
            "The empty-state branch must be inside a PullToRefreshBox -- otherwise pull-to-refresh " +
                "is unreachable while there's no post history, confirmed by inspection (2026-09-07)."
        }
